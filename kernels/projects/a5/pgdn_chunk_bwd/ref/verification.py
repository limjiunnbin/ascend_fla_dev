...
def compare_runs(left, right):
    ...
    if set(a) != {c['id'] for c in cases()}:
        raise ValueError('The complete frozen calibration grid is required')
    ...
    return dict(cases=rows, all_byte_identical=all(r['byte_identical'] for r in rows),
