...
@vf()
def diagnostic_identity_delay_vf(exp_values: Tensor, spins: Var):
    # Common timing stress: preserve all 64 FP32 values and all arithmetic.
    value = Reg(DT.float)
    for delay_index in range(spins):
        value <<= exp_values[0:1, 0:HALF_D]
        exp_values[0:1, 0:HALF_D] <<= value
        vf_barrier(VfPipe.STORE, VfPipe.LOAD)


@kernel()
def a5k03_scan_stress_ring_kernel(kg: GM[bf16, ('B', 'T', 'HV', 128)], qg: GM[bf16, ('B', 'T', 'HV', 128)], w: GM[bf16, ('B', 'T', 'HV', 128)], g_last: GM[bf16, ('B', 'C', 'HV', 128)], grad_out: GM[bf16, ('B', 'T', 'HV', 128)], Aqk: GM[bf16, ('B', 'T', 'HV', 64)], v_new: GM[bf16, ('B', 'T', 'HV', 128)], dht: GM[bf16, ('B', 'HV', 64, 256)], dAqk: GM[bf16, ('B', 'T', 'HV', 64)], dh: GM[bf16, ('B', 'C', 'HV', 128, 128)], dv: GM[bf16, ('B', 'T', 'HV', 128)], dh0: GM[bf16, ('B', 'HV', 128, 128)], B: i32, HV: i32, C: i32, SPINS: i32):
    ...
    with auto_sync():
        for bhv in range(bhv_begin, bhv_end):
            ...
            seed_mutex.lock()
            l0c_to_ub(seed_ub[(bhv - bhv_begin) * C], l0c_seed[0], M=D, N=D, N_dst=D, M_src=D, dual_mode=DualMode.SPLITM, sub_block_id=0)
            seed_mutex.ready()
            ...
            for rev_c in range(C):
                ...
                if rev_c + 1 < C:
                    ...
                    seed_mutex.lock()
                    l0c_to_ub(seed_ub[(bhv - bhv_begin) * C + rev_c + 1], l0c_seed[rev_c + 1], M=D, N=D, N_dst=D, M_src=D, dual_mode=DualMode.SPLITM, sub_block_id=0)
                    seed_mutex.ready()
                ...
                corr_mutex.wait()
                if rev_c + 1 < C:
                    update_dstate_vf(seed_ub[(bhv - bhv_begin) * C + rev_c], corr_ub, dstate_f_ub, state_h_ub, state_h_nz_ub, exp_g_last_half_ub, row_begin_d)
                else:
                    diagnostic_identity_delay_vf(exp_g_last_half_ub, SPINS)
                    update_dstate_final_vf(seed_ub[(bhv - bhv_begin) * C + rev_c], corr_ub, dstate_f_ub, state_h_ub, exp_g_last_half_ub, row_begin_d)
                corr_mutex.free()
                seed_mutex.free()
